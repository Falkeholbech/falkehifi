...
function getParameterByName(name, url) {
   ...
   function sletItem(event) {
      if (confirm('Er du sikker?')) {
         ...
         let request = new Request(`http://188.226.185.171:1337/products/${id}`, init);
   ...
   document.addEventListener("DOMContentLoaded", event => {
      ...
      if (getParameterByName('action') == "edit") {
         ...
         fetch(`http://188.226.185.171:1337/products/${productId}`)
            .then((response) => {
               if (response.ok) {
            ...
            .then((json) => {
               ...
               productFormButton.addEventListener('click', function (event) {
                  ...
                  if (id != 0 && name != '' && description != '' && !isNaN(price) && id > 0) {
                     ...
                     let url = `http://188.226.185.171:1337/products/${id}`;
                     let headers = new Headers();
                     headers.append('Content-Type', 'application/json');
         ...
         productFormButton.addEventListener('click', function (event) {
            ...
            if (name != '' && description != '' && !isNaN(price)) {
               document.querySelector('#productsFormError').innerHTML = "";
               let url = `http://188.226.185.171:1337/products/`;
               let headers = new Headers();
               headers.append('Content-Type', 'application/json');
      ...
      fetch('http://188.226.185.171:1337/products')
      .then((response) => {
         if (response.ok) {
            return response.json();
         }
